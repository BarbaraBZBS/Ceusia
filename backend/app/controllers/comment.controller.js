            ...
            if ( req.file.detectedMimeType == null ) {
                ...
                Post.findByPk( req.params.id )
                    .then( async ( post ) => {
                        ...
                        const filePath = `${ req.protocol }://${ req.get( "host" ) }/image/${ fileName }`;
                        ...
                        post.update( { discussions: db.literal( 'discussions + 1' ) } )
            ...
            .then( async ( post ) => {
                ...
                Comment.findOne( { where: { id: req.params.cid, post_id: post.id } } )
                    .then( async ( comment ) => {
                        ...
                        if ( req.auth.role != 'admin' && comment.user_id != req.auth.user_id ) {
...
export { createComment, getAllPostComments, findOnePostComment, updateComment, deleteComment };
