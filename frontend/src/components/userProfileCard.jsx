...
export default function UserProfileCard( { user } ) {
    ...
    return (
        <>
            { isLoading ? <Loading /> :
                <div className='flex flex-col justify-center items-center pt-8 pb-10 mt-10 mb-20 min-h-[400px] border-gray-900 bg-apppastgreen bg-opacity-30 rounded-lg shadow-lg w-[90%] mx-auto'>
                    <div>
                    ...
                    <div className='flex justify-center items-center w-[140px] h-[140px] rounded-full relative mb-5 transition-all duration-300 ease-in-out delay-75 hover:scale-105 hover:bg-apppink hover:drop-shadow-light' onClick={ () => showUsrPicZoomOverlay() }>
                        <Image title={ `zoom ${ user.username } picture` } width={ 0 } height={ 0 } priority src={ user.picture } alt={ `${ user.username } picture` } placeholder="empty" className='rounded-full object-cover w-full h-full cursor-pointer' />
                    </div>
                    <div className={ isPicZoomed ? 'fixed overflow-y-scroll left-0 right-0 top-0 bottom-0 w-full h-full bg-appblck z-[998] flex' : 'hidden' } onClick={ () => hideUsrPicZoomOverlay() }>
                        { isPicZoomed && <Image width={ 0 } height={ 0 } priority placeholder='empty' src={ user.picture } alt={ `${ user.username } picture` } className='block m-auto w-[96%] aspect-square object-cover rounded-full animate-resizeZoom' /> }
                    </div>

                    { followed ? <div>
                        <p className='text-clamp8 pt-1 text-appmauvedark'> You are following { user.username }.</p>
                    </div> : <div className='text-clamp8 pt-1 text-gray-400'> You don`&apos;`t follow { user.username }.</div>
                    }
                    { followingUsr ? <div>
                        <p className='text-clamp8 pb-3 text-appmagenta'> { user.username } is following you.</p>
                    </div> : <div className='text-clamp8 pb-3 text-gray-400'>{ user.username } does not follow you.</div>
                    }
                    ...
                    <div className='flex w-[80%] items-center justify-evenly'>
                        <div>
                            <button title={ postId ? 'back to the post' : 'back to posts' } onClick={ () => backToPost( `/#${ postId }` ) } onAnimationEnd={ () => setBackBtnEffect( false ) }
                        ...
                        { followed ?
                            <div className=''><button title='unfollow user' onClick={ () => handleUnfollow() } onAnimationEnd={ () => setUnfollowEffect( false ) }
                                className={ `bg-appstone text-clamp6 text-white w-[41px] h-8 rounded-xl px-2 py-1 transition-all duration-300 ease-in-out hover:bg-appopred hover:border-appopred hover:text-appblck hover:translate-y-[7px] hover:shadow-btnblue ${ unfollowEffect && 'animate-btnFlat text-appblck bg-apppastgreen' }` }>
                            ...
                            : <div><button title='follow user' onClick={ () => handleFollow() } onAnimationEnd={ () => setFollowEffect( false ) }
                                className={ `bg-appstone text-clamp6 text-white w-[41px] h-8 rounded-xl transition-all duration-300 ease-in-out hover:bg-appopred hover:border-appopred hover:text-appblck hover:translate-y-[7px] hover:shadow-btnblue ${ followEffect && 'animate-btnFlat text-appblck bg-apppastgreen' }` }>
