...
import HomeFollows from './homeFollows';

export default function Cards( { posts, users, session, params, searchParams } ) {
    ...
    return (
        <>
            <TrendingCards trendShown={ trendShown } setTrendShown={ setTrendShown } setMeetShown={ setMeetShown } posts={ posts } setposts={ setDisplay } session={ session } />
            <SuggestionsCard meetShown={ meetShown } setMeetShown={ setMeetShown } setTrendShown={ setTrendShown } users={ users } session={ session } allFollowers={ allFollowers } setAllFollowers={ setAllFollowers } allFollowings={ allFollowings } setAllFollowings={ setAllFollowings } />
            <PageWrap>
                <div className='flex flex-col items-center'>
                    <div className='mt-16 mb-4 flex flex-col items-center w-[331.2px]'>
                        {/* follows */ }
                        <HomeFollows session={ session } allFollowers={ allFollowers } setAllFollowers={ setAllFollowers } allFollowings={ allFollowings } setAllFollowings={ setAllFollowings } />
                        {/* post form */ }
                        <h3 className='text-clamp7 font-medium'>What&apos;s on your mind?</h3>
                        ...
                        <div key={ post.id } id={ post.id } className='w-full h-full'>
                            <AnimatePresence mode='wait'>
                                <motion.div
                                    ...
                                    <div onClick={ () => { setClickedBtn( index ); goToPost( `/coms/${ [ post.id ] }` ) } }>
                                        <p className={ `cursor-pointer line-clamp-3 text-clamp1 mx-3 mt-2 mb-3 ${ clickedBtn === index && goToPostEffect && 'opacity-50' }` }>{ post.content }</p>
                                    </div>
                                    ...
                                    { post.link && <LinkVideo postLink={ post.link } /> }
                                    { session?.user.user_id === post.user_id || session?.user.role === 'admin' ? <div className='flex justify-end mb-1 mx-6'>
                                    ...
                                    <div className='flex justify-between'>
                                        ...
                                        {/* share post */ }
                                        <div className={ `mr-4 hover:opacity-50 ${ clickedBtn === index && shareEffect && 'animate-resizeBtn' }` } onAnimationEnd={ () => setShareEffect( false ) }>
                                            <button title='share post' onClick={ () => { setClickedBtn( index ); handleShare() } }><FontAwesomeIcon icon={ faShareFromSquare } style={ { color: '#7953be' } } /></button>
                                        </div>
                                    </div>
                                    <div className='flex flex-col items-center justify-center my-1 text-clamp2 font-extralight'>
                                        { post.editedAt ? ( post.editedByAdmin ? <p className=''>Edited <span className='font-medium text-indigo-500'> BY ADMIN</span> { dateParser2( post.editedAt ) }</p> : <p className=''>Edited { dateParser2( post.editedAt ) }</p> ) : <p></p> }
                                    </div>
                                </motion.div>
                            </AnimatePresence>
                        </div>
