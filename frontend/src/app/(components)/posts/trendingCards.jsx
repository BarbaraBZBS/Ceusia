	...
	return (
		<>
			{trendShown ? (
				<button
					title="hide trending"
					className="bg-apppastgreen mb-[0.4rem] border-2 border-apppastgreen rounded-tl-lg rounded-bl-lg pl-[0.8rem] pr-[0.4rem] hover:text-appmauvedark shadow-strip"
					onClick={() => hideTrend()}>
					<FontAwesomeIcon
						icon={faChevronRight}
						...
						onAnimationEnd={() => setHideTrendingEffect(false)}
					/>
				</button>
			) : (
				<button
					title="show trending"
					className="bg-apppastgreen mb-[0.4rem] border-2 border-apppastgreen rounded-tl-lg rounded-bl-lg pl-[0.8rem] pr-[0.4rem] hover:text-appmagenta shadow-strip"
					onClick={() => showTrend()}>
					<FontAwesomeIcon
						icon={faChevronLeft}
						className={`text-clamp1 ${
						...
						onAnimationEnd={() => setShowTrendingEffect(false)}
					/>
					<span className="text-clamp2 ml-[0.4rem]">Trending</span>
				</button>
			)}
			<AnimatePresence>
				{trendShown && (
						...
						<div
							...
							{trending.map((post, index) => (
								<div
									...
									<div
										className={`bg-white w-[96%] mx-auto border-2 border-white rounded-xl shadow-card my-[0.8rem] relative`}>
										<div className="flex text-clamp6 mx-[1.2rem] mt-[0.4rem] mb-[0.3rem]">
											<div className="flex items-end">
												...
												post.user_id ? (
													...
													<a
														...
														className={`mb-[0.5rem] hover:text-appturq active:text-appturq ${
															clickedBtn ===
																index &&
															usrLinkEffect &&
															"animate-resizeBtn"
										...
										<div className="flex mx-[1.2rem] items-center">
											...
											{post.fileUrl &&
												post.fileUrl?.includes(
													...
													<div className="flex w-[9rem] min-w-[9rem] aspect-square">
														<video
															id={post.id}
															width="100%"
															height="100%"
											...
											<div className="flex flex-col ml-[1.2rem]">
												{post.title ? (
													<div className="">
														<h2 className="text-clamp7 line-clamp-1 font-medium mx-[1.2rem]">
															{post.title}
														</h2>
													</div>
												) : (
												...
												<div>
													<p className="line-clamp-2 text-clamp1 mx-[1.2rem] mt-[0.8rem] mb-[1.2rem]">
														{post.content}
													</p>
												</div>
											</div>
